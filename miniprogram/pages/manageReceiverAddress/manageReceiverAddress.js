// pages/manageReceiverAddress/manageReceiverAddress.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.cloud.callFunction({
      name: "getReceiverAddresses",
      success: (res) => {
        console.log(res.result.data);
        this.setData({
          receiverAddresses: res.result.data
        })
  ...
  onUnload: function () {
    ...
    {
      console.log(999);
      for (let index = 0; index < this.data.receiverAddresses.length; index++) {
        const element = this.data.receiverAddresses[index];
        wx.cloud.callFunction({
